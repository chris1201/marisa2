...
class PopCount {
 public:
  PopCount() = default;
  explicit constexpr PopCount(std::uint32_t x) : value_(pop_count_1st(x)) {}

  explicit constexpr operator bool() {
    return value_ != 0;
  }

  constexpr std::uint8_t low_8() {
    return static_cast<std::uint8_t>(value_);
  }
  constexpr std::uint8_t low_16() {
    return static_cast<std::uint8_t>(value_ >> 8);
  }
  constexpr std::uint8_t low_24() {
    return static_cast<std::uint8_t>(value_ >> 16);
  }
  constexpr std::uint8_t low_32() {
    return static_cast<std::uint8_t>(value_ >> 24);
  }

  static constexpr std::uint8_t pop_count(std::uint32_t x) {
 ...
 private:
  ...
  static constexpr std::uint32_t pop_count_1st(std::uint32_t x) {
    return pop_count_2nd((x & 0x55555555U) + ((x & 0xAAAAAAAAU) >> 1));
  }
  static constexpr std::uint32_t pop_count_2nd(std::uint32_t x) {
    return pop_count_3rd((x & 0x33333333U) + ((x & 0xCCCCCCCCU) >> 2));
  }
  static constexpr std::uint32_t pop_count_3rd(std::uint32_t x) {
    return pop_count_last((x & 0x0F0F0F0FU) + ((x & 0xF0F0F0F0U) >> 4));
  }
  static constexpr std::uint32_t pop_count_last(std::uint32_t x) {
